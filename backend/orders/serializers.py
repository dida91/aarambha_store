...
class OrderStatusHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'from_status', 'to_status', 'actor', 'actor_username', 'note', 'created_at']
...
class CheckoutSerializer(serializers.Serializer):
    ...
    def create_order(self, user):
        ...
        shipping_address = ShippingAddress.objects.filter(user=user, id=self.validated_data['shipping_address_id']).first()
        ...
        if not shipping_config:
            raise serializers.ValidationError({'shipping_config': 'No active shipping config.'})
        ...
        for item in cart_items:
            ...
            if not product.is_active:
                raise serializers.ValidationError({'product': f'{product.name} is not active.'})
        ...
        if promo_code:
            promo_obj = PromoCode.objects.select_for_update().filter(code__iexact=promo_code).first()
            if not promo_obj:
...
class SellerOrderStatusUpdateSerializer(serializers.Serializer):
    status = serializers.ChoiceField(choices=[Order.Status.CONFIRMED, Order.Status.REJECTED])
    ...
    def validate(self, attrs):
        ...
        if order.status != Order.Status.PENDING:
            raise serializers.ValidationError('Only PENDING orders can be transitioned.')
        if attrs['status'] == Order.Status.REJECTED and not attrs.get('rejection_reason', '').strip():
            raise serializers.ValidationError({'rejection_reason': 'Rejection reason is required for rejected orders.'})
        return attrs
